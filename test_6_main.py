...
simulation_time_period = range(0, 10)

# Simulation of the network efficiency over 100 years
for t in simulation_time_period:

    pci_samples = []                        # this should  be a two-dimensional list
    velocity_samples = []
    time_samples = []
    ...
    for _ in range(2):
        ...
        #temp_pci_sample = []
        temp_pci_sample = [[] for _ in range(len(road_network_1.edges()))]                 # With empty sublist

        for u, v, data in temp_network.edges(data=True):                  # enumerate gibt eine Liste von Kanten zurück
        ...
        #pci_samples.append(temp_pci_sample)
        pci_samples = temp_pci_sample
    ...
    # PCI_mean = [sum(sublist) / len(sublist) for sublist in pci_samples]
