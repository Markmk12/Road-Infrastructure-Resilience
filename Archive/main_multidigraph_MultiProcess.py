...
def process_strategy(strategy):
    ...
    return {
        'strategy_idx': idx,
        'efficiency': mean_efficiency_row,
        'resilience': resilience,
        'costs': np.sum(mean_costs_row, axis=0),
        'costs_history': mean_costs_row
    }

# Definieren der Anzahl der Prozesse, die verwendet werden sollen
# verwendet werden standardmäßig so viele Prozesse wie verfügbare CPU-Kerne

num_processes = cpu_count()
# num_processes = 1

# Erzeugen eines multiprocessing Pools
if __name__ == '__main__':
    with Pool(processes=num_processes) as pool:
        print("Starting multiprocessing pool...")
        results = pool.map(process_strategy, all_strategies)
        print("Multiprocessing pool finished.")

    # Speichern der Ergebnisse in den ursprünglichen Matrizen
    for result in results:
        idx = result['strategy_idx']
        strategies_matrix_efficiency[idx, :] = result['efficiency']
        strategies_matrix_resilience[idx] = result['resilience']
        strategies_matrix_costs[idx] = result['costs']
        costs_history_matrix[idx] = result['costs_history']
