...
# Quality levels of road maintenance (the quality level 'none' is not in use but defined in the maintenance functions)
...
corrective_history_matrix = np.zeros((len(all_strategies), len(simulation_time_period)))
...
for idx, strategy in enumerate(all_strategies):
    ...
    end0 = time.time()
    print("Execution time of one strategy: ", str(end0 - start0), "[sec]")
...
if not best_strategy:
    ...
    # Expected number of preventive measures of the best strategy path
    best_preventive_count = np.around(preventive_history_matrix[idx_best])
    best_preventive_count_cumulated = np.cumsum(best_preventive_count)

    # Expected number of corrective measures of the best strategy path
    best_corrective_count = np.around(corrective_history_matrix[idx_best])
    best_corrective_count_cumulated = np.cumsum(best_corrective_count)
    ...
    np.save(os.path.join('results', file, 'best_preventive_count.npy'), best_preventive_count)
    np.save(os.path.join('results', file, 'best_corrective_count.npy'), best_corrective_count)
...
np.save(os.path.join('results', file, 'corrective_history_matrix.npy'), corrective_history_matrix)
...
# Bar chart with zorder=2
plt.bar(range(len(best_strategy_costs)), best_strategy_costs, color='blue', label='Costs', zorder=2)
